...
staying high is optimal.  We prove a separate finite-time boundary: after a
learner's downstream value estimates become usable, if it chooses the
exploratory cut with conditional probability at most $q$, observing a fresh
length-$M$ cut path within $T$ further steps has probability at most $Tq^M$.
Thus the memory that defeats a fixed late-learning horizon grows only
logarithmically in $T$, even when rewards have high signal.  A path-equivalent
commitment option reduces the corresponding path proposal time to linear while
preserving every primitive price path and the optimal value.
...
$[-0.0755,-0.0493]$).  A trace replay shows that all 20 primitive learners
reached the all-low state within 901 early steps, but none of the 19 failures
reached even depth five after step 100,000.  The intervention reverses beyond
...
terminal Q-table against the frozen artifact to tolerance $10^{-12}$, and
records trailing-low-state visits after steps 100,000 and 200,000.  These
checkpoints are diagnostics, not preregistered readiness estimates.
...
901.  Nevertheless, 19 finish with the wrong high initial action.  Among those
19 failures, none visits a state with five or more trailing low actions after
step 100,000, and none returns to the all-low state.  Thus the naive
first-discovery interpretation is falsified.  The observed mechanism is
consistent with \emph{late credit starvation}: informative states are sampled
while exploration is broad, but are not revisited deeply enough after value
estimates separate.  This diagnostic motivates the stopping-time formulation
of \cref{thm:critical}; it does not estimate a unique readiness time.
...
logarithmic improvement from longer training.
