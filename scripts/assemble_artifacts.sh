...
mkdir -p "$DEST"
count=0
for wf in capture.yml capacity-policy-probes.yml enforcement-policy-probes.yml decomposition-probes.yml decomposition-replication.yml route-calibration.yml paid-price-response.yml price-event-probes.yml market-measurement.yml glm52-routing.yml score-memory-routing.yml score-memory-quality.yml adaptive-router.yml evals.yml gpu.yml hf-router.yml hf-policy-probes.yml livepeer.yml probes.yml router-catalogs.yml watchers.yml; do
  limit=40
  ...
  if [ "$wf" = "glm52-routing.yml" ] || [ "$wf" = "score-memory-routing.yml" ]; then
    limit=120
  fi
