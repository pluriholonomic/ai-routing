...
routing probes.  Three facts characterize this market.  First, prices are
...
Third, the router manufactures firmness from revocable quotes.  In a
randomized-crossover release, default routing succeeds on 151 of 152 requests,
while three pinned single-provider policies succeed on 61--64 of 76; rejection
is flat in price rank, refuting the stale-cheap last-look pattern suggested by an
earlier fixed-order pilot.  A steering audit provides the mechanism link:
...
The paper's contribution is three empirical facts and one mechanism audit.
...
\begin{enumerate}
  ...
  \item \textbf{Manufactured firmness.} The money quote barely moves while
  ...
  blocks.  Default routing succeeds at 99.3\%; pinned routes succeed at
  80.3--84.2\%.  The common rejection level survives randomization, but an earlier
  apparent price-rank gradient does not.  Individual quotes are revocable for the
  studied account, and the router's fallback option creates the delivered product.
...
Randomized crossover & 380 & prospective & Quote firmness and policy-order correction \\
...
\section{Fact 3: the router manufactures quote firmness}
\label{sec:firmness}
...
The first released accrual has 152 default attempts and 76 attempts in each pinned
arm.  Default succeeds on 151 of 152 requests, or 99.3\%.  Pinned-cheapest succeeds
on 61 of 76 (80.3\%), pinned-second on 62 of 76 (81.6\%), and pinned-random on 64 of
76 (84.2\%).  Marginal Wilson intervals are shown in \cref{fig:core}; repeated
model-time blocks mean these intervals are descriptive, not the final
randomization inference.  The registered promotion gate requires 500 attempts per
pinned arm, Holm-adjusted default-pinned contrasts, seed replay, and the
first-position sensitivity.

The economically important result is the combination of a level and a null.  The
approximately 19\% pinned rejection level survives randomization.  The ranking
gradient does not: the cheapest, second-cheapest, and random pinned endpoints have
similar success.  The pilot's apparent stale-cheap ``last look'' was an order
artifact.  The surviving fact is provider-level revocability plus platform-level
firmness.  A visible quote is an eligibility advertisement rather than a firm
commitment for this account, and the router assembles a reliable product by
substituting among individually unreliable sources.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/core_facts.pdf}
\caption{Four frozen v3 facts.  Panel C shows marginal Wilson 95\% intervals;
the registered policy contrasts use block randomization inference at the
500-per-pinned-arm gate.  Panels A, B, and D are descriptive panel statistics.}
\label{fig:core}
\end{figure}
...
The central market-design implication follows directly from Fact 3.  A price-only
...
both-vintage re-estimation.  Second, the randomized crossover is an interim release
and has not reached 500 attempts per pinned arm.  Third, owned probes identify one
...
sticky administered menus, minimum quotes coordinate around the model author's
price, and the platform manufactures a firm service by substituting across
revocable provider quotes.  The platform's allocation memory does not reward
recent undercutting in the observed panel.  These facts make the inference router
a market designer: its admission, fallback, and steering rules determine the
delivered product as much as the visible price surface does.
...
the paper's three facts because its market object is a compute lease rather than a
...
Manufactured firmness & Randomized level, flat rank gradient & Full randomization precision and transport & 500 attempts per pinned arm \\
...
All tables are generated from versioned Parquet and JSON artifacts.  Capture rows
carry source timestamps, retrieval timestamps, and source-run identifiers.  Probe
assignments include a study identifier, block identifier, seed, intended policy,
position, and quote-snapshot key.  Prompt payloads and secrets are excluded.  The
repository contains the analyzers, preregistrations, claim-boundary documents, and
machine-readable summaries.  The four-panel figure reads only the frozen
\texttt{paper\_estimates.json}; it intentionally refuses the local legacy H80
summary because that file predates the randomized release.  At each registered
gate, the paper estimates, source manifest, and rendered PDF must be updated in the
same commit.
