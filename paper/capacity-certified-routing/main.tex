...
An exact official-ID bridge recovers 281 legacy request-to-quote matches but
zero capacity-complete public states, and a prospective capacity-policy run
finds no eligible pair because the public capacity fields are null. A
separately preregistered enforcement-stress trial does enroll: three remote
runs yield 23 valid hot-model assignments across low-stress, high-stress, and
default routing. Those arm outcomes remain masked.
To test whether the delegation frontier transports across routers, we also
preregister and launch an independent Hugging Face policy trial. It randomizes
documented fastest and cheapest routing against a pinned public cost-caliper
frontier. The first published audit has 44 valid assignments across 11 models
and eight public candidate providers, with exact seed replay and treatment
compliance; its cost, latency, success, and selected-provider outcomes remain
masked until a fixed 72-hour gate.
...
This paper isolates that distinction. It makes nine contributions.
\begin{enumerate}
  ...
  interpretation in this discovery panel. An exact request-to-public-state
  bridge then documents zero capacity-complete matches. We preserve that
  failed capacity trial and launch a separately preregistered, outcome-masked
  randomized enforcement-stress policy trial.
  \item We give an independent cross-router randomization on Hugging Face's
  documented fastest, cheapest, and explicit-provider interfaces. The third
  arm chooses the highest-throughput public provider within a 25\% total-quote
  caliper. This trial separately tests whether the measured delegation frontier
  is an OpenRouter-specific artifact; its first 44 valid assignments remain
  outcome-masked under a frozen 72-hour release rule.
\end{enumerate}
...
Hugging Face's documented inference-provider interface exposes server-side
fastest and cheapest suffixes and an explicit-provider suffix
\cite{hfinferenceproviders2026}. We use those controls only in a separate
owned-request randomization; the interface remains a proxy and billing layer,
not a direct capacity contract.
...
Execution-state bridge & 281 exact legacy request/quote matches; zero capacity-complete states & public-capacity support failure only \\
Randomized enforcement policy & 23 valid hot-model blocks; 14 providers; outcomes masked & feasibility and assignment support only \\
Randomized HF policy frontier & 44 valid blocks; 11 models; eight public candidate providers; outcomes masked & independent cross-router assignment support only \\
Simulated share $\hat s_i$ & documented public price proxy & mechanical comparative static only \\
Selection / retries & 716 legacy attempts plus 98 OpenRouter and 44 HF outcome-gated prospective rows & account/workload policy calibration only \\
...
the holdout, adding reaction features reduces MAE from 0.1179 to 0.1123 and
RMSE from 0.2152 to 0.2139, while the separate strategic hazard model improves
log loss but reduces AUC. This small predictive gain does not repair the absent
frozen reaction exposure, and the 30-day gate therefore fails.
...
\subsection{Capacity-state execution bridge and enforcement-policy trial}

The next preregistered step asks whether public capacity state predicts the
realized success of an owned, pinned request. H86 attempts an exact backward
join from 348 legacy pinned attempts to the contemporaneous public model and
provider state. It produces zero matches because the request log retains the
API model ID while the public operational panel uses a versioned canonical
slug. We do not repair that join after inspecting outcomes.

H86b separately freezes the official historical
\texttt{models\_snapshots.id}-to-\texttt{canonical\_slug} mapping, requires a
pre-request mapping no older than seven days, and preserves exact provider
names. It maps all 348 attempts and recovers 281 exact backward quote/provider
states; 45 lack an exact provider and 22 have no recent official model map.
However, all 281 matched states lack a public capacity ceiling and recent peak,
so no capacity-risk pair or outcome contrast exists. The prospective H87
capacity-policy collector confirms the support failure: its first eight-model
run sends zero requests, and an outcome-free top-40 census finds no model with
two capacity-complete providers. H86b and H87 are missing-treatment-support
results, not zero effects.

H88 is a new trial, committed before its first request. It uses the public
five-minute counts that remain populated and defines
\[
 e_{it}=\frac{\text{rate-limited}_{it}+1}
 {\text{success}_{it}+\text{rate-limited}_{it}+2}.
\]
Within each model-run, it requires at least ten public attempts per provider,
excludes deranked providers, and selects the largest low/high $e_{it}$ gap
among provider pairs whose completion prices differ by at most 25\%. It then
randomizes exactly one first-and-only request among a pinned low-stress
provider, a pinned high-stress provider, and default routing. The co-primary
intention-to-treat contrasts are low-minus-high success and default-minus-low
success. The first release is the earliest prefix with 28 complete days, 150
valid assignments per arm, ten models, twenty candidate providers, at most
20\% requested-provider dominance, at least 90\% pinned compliance, exact seed
replay, and no cross-study request within five minutes.

Across three remote runs, the merged support panel contains 24 candidate
model-blocks; 23 are eligible and each sends one request. The assignment split
is seven low-stress, nine high-stress, and seven default. Fourteen distinct
providers enter the candidate pairs. The median stress gap is 0.342 while the
median high/low price ratio is 1.000. The authoritative merged audit has exact
seed replay, 100\% pinned compliance, and no cross-study overlap, but requested-
provider dominance is 31.25\%, above the frozen 20\% gate. The panel still has
only eight models and zero complete days, so the duration, arm-count, model,
provider-diversity, and concentration gates remain open. No arm success,
rejection, latency, selected-provider, or cost result is released.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/h88_enforcement_policy_support.pdf}
\caption{Outcome-free H88 enrollment support. Points show the public
safe/risky candidate-pair stress gap and completion-price ratio, colored by the
randomized single-request assignment. The dashed line is the frozen 1.25 price
caliper. The bars report assignments only; no arm outcome is displayed.}
\label{fig:h88support}
\end{figure}

H88 can identify an account-level policy effect for acting on a public router
rejection state. It cannot identify physical capacity, provider cost or
intent, literal front-running, other-user flow, or the welfare effect of the
capacity-certified mechanism.

\subsection{Independent Hugging Face policy-frontier trial}

H89 asks whether the cost--speed--reliability delegation problem transports to
a second router with a different documented policy interface. Hugging Face
documents server-side \texttt{:fastest} and \texttt{:cheapest} suffixes and an
explicit provider suffix \cite{hfinferenceproviders2026}. Its response exposes
the realized provider in the \texttt{x-inference-provider} header for the owned
request. This is stronger execution telemetry than a public routing simulation,
but Hugging Face remains in the request and billing path.

Before confirmatory enrollment we froze twelve literal chat-model IDs and a
one-token, temperature-zero workload. A model-run is eligible only when at
least two live providers report nonnegative input and output prices plus
positive throughput, and the public cheapest and fastest providers differ.
Each eligible model-run receives one arm with probability $1/3$:
\texttt{:fastest}, \texttt{:cheapest}, or an explicit provider selected by
\[
 i_{\delta}\in\arg\max_i\{t_i:c_i\leq(1+\delta)c_{\min}\},
 \qquad \delta=0.25,
\]
where $c_i$ is the public 64-input/one-output quote cap and $t_i$ is reported
throughput. Thus the experimental arm is the public throughput maximizer within
a fixed cost caliper; this is a declared policy optimization, not an estimate
of provider cost or private queue state.

The published outcome-free audit through 2026-07-15 21:12:56 UTC contains 48
candidate model-runs, 44 eligible and valid assignments, and eleven distinct
models. Arm counts are 10 fastest, 16 cheapest, and 18 cost-caliper. Eight
public candidate providers enter; the largest public predicted provider share
is 54.5\%. Treatment compliance and seed replay are both 100\%. The panel spans
three elapsed hours. The release rule requires 72 elapsed hours, 120 valid
assignments per arm, eight models, five candidate providers, at most 75\%
public predicted-provider dominance, at least 95\% compliance, and exact seed
replay. It releases the earliest qualifying chronological prefix regardless of
sign. No arm success, failure, latency, estimated cost, or realized-provider
aggregate is reported before that cut.

At release, four two-sided randomization tests compare fastest versus cheapest
failure-penalized latency, cheapest versus fastest cost, and the cost-caliper
arm versus each server policy under a frozen generalized loss. Model-day
cluster intervals accompany Holm-adjusted randomization $p$-values. The
generalized loss fixes a value of time and failure penalty; it is an owner
objective, not social welfare or willingness to pay.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/h89_hf_policy_support.pdf}
\caption{Outcome-free H89 enrollment support. The left panel reports randomized
assignment counts. The right panel reports the public predicted provider used
to audit support and concentration; it is not the realized provider outcome.}
\label{fig:h89support}
\end{figure}
...
utilization predict the next event, but the literal capacity-policy experiment
has no current public-field support. The separately frozen H88 trial can test
an observable admission-risk policy without relabeling that missing capacity
state. The running H80/H81/H88 randomizations and outcome-masked H83/H85 future
holdouts make the unresolved claims falsifiable without pooling discovery and
confirmation data. H89 adds an independent router and a directly auditable
policy assignment, but it still cannot create provider commitment, collateral,
or collectable shortfall liability. Its outcome gate therefore strengthens the
delegation evidence without filling the paper's L4 capacity primitive.
