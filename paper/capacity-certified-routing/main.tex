...
Selection / retries & 716 legacy attempts plus 72 outcome-gated prospective rows & account/workload policy calibration only \\
...
The latest pre-outcome audit contains 12 verified H80 position-zero blocks
across four models and three hourly clusters; each model contributes three
blocks. H81 contains four verified blocks across two models and two runs. All
12 H80 seed replays and all four H81 seed and treatment-control replays pass.
Arm counts are $(3,1,2,6)$ for H80 and $(1,1,2)$ for H81, so both analyzers
remain below their 40-per-arm gates and release no outcome, confidence interval,
or $p$-value. The H81 candidate-funnel table begins after these four blocks;
the earlier support is therefore correctly labeled left-truncated rather than
used to infer which public-rank candidates were excluded.
