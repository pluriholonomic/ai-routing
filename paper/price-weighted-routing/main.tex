...
\begin{enumerate}
  ...
  eight-step TD falsification does not reproduce the improvement.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \]
  \item If $a(n-1)\le n$, the only symmetric equilibrium is the menu ceiling.
...
rivals, executable open-source routers, and longer calibration panels are future
tests.  No result establishes provider intent, communication, collusion, or a
...
interior profile is positive and the cap is the only symmetric solution.
...
\texttt{eb19ae1}, a result hash, input-artifact hashes, and equality between the
