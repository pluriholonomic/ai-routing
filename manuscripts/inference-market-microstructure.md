...
inference. Harnesses such as Hermes shape demand, routers such as OpenRouter
aggregate requests and privately dispatch execution, and providers sell capacity
under common model labels. Using five-minute quotes and randomized
micro-purchases, we find sticky administered menus, frequent exact price ties, and
revocable quotes whose delivery is strengthened by router substitution. Slow
repricers charge more, echoing Brown–MacKay, but hard tests do not identify rival
response or front-running. The market therefore resembles dealer/RFQ liquidity
combined with reusable-capacity dispatch, not an automated market maker. A
benchmark shows that efficient routing requires prices augmented by reliability,
delay, quality, and scarcity, and that free entry is finite but can be excessive
through business stealing or insufficient when providers cannot capture
redundancy value.
