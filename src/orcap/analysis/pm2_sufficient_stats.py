...
def changes() -> pd.DataFrame:
    df = data.q(
        f"""
        select changed_at_run_ts, model_id, provider_name, endpoint_fingerprint,
...
def run(out_dir: Path = DEFAULT_OUT) -> dict:
    ...
    if len(ch) < 50:
        ...
        return summary
    save(ch.assign(ts=ch["ts"].astype(str), prev_ts=ch["prev_ts"].astype(str)), out_dir, "pm2_changes")
    d = ch["dlog"].to_numpy()
    ...
    n_freq = float(len(ch) / endpoint_days())  # changes per endpoint-day
