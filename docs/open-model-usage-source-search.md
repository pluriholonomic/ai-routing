...
| [Nosana Markets API](https://learn.nosana.com/api/markets.html) | Its market schema would expose GPU type, price, resource requirements, availability, and queue state for a distinct decentralized GPU market. The official API introduction requires a valid API key or wallet authentication for all requests. | Hold behind an explicit user-provided `NOSANA_API_KEY`; do not reuse a browser session or public frontend credential. If configured, treat market availability/queue and USD reward fields as source-defined state, not delivered jobs, utilization, or an all-market clearing price. |
| [Nosana Nodes on-chain registry](https://docs.nosana.com/nodes/introduction) | The public Solana NodeAccount program permits a credential-free, slot-pinned header query with declared hardware-profile fields and the audited flag. It does not expose liveness, GPU model/count semantics, free capacity, queue, price, utilization, jobs, or delivered compute. | The monitor captures the 54-byte declared header through public Solana JSON-RPC and fails closed unless raw account and parsed-node counts agree. H58 can report repeated declared-registry composition only; do not merge these fields into capacity, demand, or cross-venue price comparisons. |
| [Nosana Explore aggregate job API](https://explore.nosana.com/) | The public Explore UI calls unauthenticated aggregate endpoints for cumulative job state, market-level running counts, and rolling source-reported job-count and job-duration buckets. | The monitor calls only aggregate endpoints and retains no job definitions, result payloads, payer IDs, or per-job metadata. H59 can report source-defined job activity after its dynamic-panel gate; do not call the UI's duration label independently verified GPU-hours, or interpret it as LLM routing, tokens, capacity, utilization, revenue, or causal demand. |
