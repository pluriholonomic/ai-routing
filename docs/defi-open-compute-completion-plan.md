...
| Direct inference basis | structured daily DeepInfra, Cerebras, SambaNova, and Chutes catalogs; Groq/Together exact-ID docs tables; Novita's literal-ID public SSR catalog; and two Fireworks exact-ID serverless pages | Groq, Together, Novita, and Fireworks are posted quotes. Chutes's TEE-suffixed catalog IDs require a versioned root/quantization-verified map before joining OpenRouter canonical IDs. Coverage remains too narrow for a market-wide conclusion. |
...
| OpenRouter direct providers | direct vs routed venue basis | DeepInfra, Cerebras, SambaNova, and Chutes structured APIs plus Together, Groq, Fireworks, and Novita typed adapters are implemented. Chutes maps its TEE-suffixed direct IDs only when a versioned literal root and quantization pair agrees with a current Chutes OpenRouter endpoint. Novita's parser accepts only active chat models with exact API IDs and displayed USD-per-million prices from its named public SSR catalog. Lambda's token Inference API is winding down; qualify any other provider with a public structured model/pricing endpoint next. Hyperbolic's unauthenticated model API returned 401 and its older pricing pages now redirect, so it requires a provider-approved public feed rather than an account-specific billing workaround. Archive HTML only as fallback evidence. |
