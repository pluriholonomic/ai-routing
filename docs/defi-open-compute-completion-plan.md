...
| Direct inference basis | structured daily DeepInfra prices, Groq/Together exact-ID docs tables, Novita's literal-ID public SSR catalog, and two Fireworks exact-ID serverless pages | Groq, Together, Novita, and Fireworks are posted quotes; other provider pages remain raw HTML only, so coverage is still too narrow for a market-wide conclusion |
...
| OpenRouter direct providers | direct vs routed venue basis | DeepInfra, Cerebras, and SambaNova structured APIs plus Together, Groq, Fireworks, and Novita typed adapters are implemented. Novita's parser accepts only active chat models with exact API IDs and displayed USD-per-million prices from its named public SSR catalog. Qualify Lambda, Hyperbolic, and any provider with a public structured model/pricing endpoint next. Archive HTML only as fallback evidence. |
