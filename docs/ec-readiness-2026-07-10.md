...
| Nosana declared node registry | H58 has a public, slot-pinned Solana NodeAccount header collector with raw-to-parsed completeness checks; a read-only 2026-07-10 validation returned 19 unique parsed headers at one slot | One valid local snapshot only (power-gated at 1/7 days and 1/20 snapshots); declarations are not live availability, GPU count/model, price, utilization, jobs, delivered capacity, or a cross-venue capacity measure |
| Nosana aggregate job activity | H59 has an aggregate-only public Explore API collector: state counts, market-level running counts, and rolling count/duration buckets; a local source-run validated 25 buckets per series and 34 markets without collecting individual jobs | One valid local snapshot only (power-gated at 2/7 bucket days and 25/100 buckets per series); duration remains a source-defined aggregate rather than verified GPU-hours, LLM demand, routing, utilization, revenue, or a capacity measure |
