...
| `market-capture` | `market_quotes`, `market_counterfactual_quotes` | hourly workflow | Includes GeckoTerminal indexed state as a reserve/volume control, six-point Uniswap QuoterV2 price-impact curves plus 100/500-bps sparse-ladder capacity lower bounds, and exact-input parent-block AMM counterfactuals for observed CoW USDC sells. The lower bounds are not full tick-book depth; neither simulation is a fill guarantee or best-execution result. |
