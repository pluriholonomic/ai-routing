...
| `market-capture` | `market_quotes`, `market_counterfactual_quotes`, `uniswap_tick_book`, `akash_market_open_bids` | twice-hourly workflow | Includes GeckoTerminal indexed state as a reserve/volume control, six-point Uniswap QuoterV2 price-impact curves plus 100/500-bps sparse-ladder capacity lower bounds, a full finality-block-pinned initialized-tick state map for the registered Uniswap pools, exact-input parent-block AMM counterfactuals for observed CoW sells, and block-pinned open GPU bids from the current Akash live-GPU-provider universe. Tick state and lower bounds are not full dollar depth; neither simulation nor an Akash bid is a fill guarantee or best-execution result. |
