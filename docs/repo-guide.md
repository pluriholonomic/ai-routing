...
| `capture-direct` | `direct_prices_daily` | daily | DeepInfra, Cerebras, SambaNova, and Chutes structured public model APIs; Groq and Together exact-ID docs tables; a strict Novita public SSR catalog with literal API IDs; and two exact-ID Fireworks serverless model pages. Cerebras retains both its API ID and a first-party canonical Hugging Face key when supplied; SambaNova uses versioned one-to-one maps; Chutes needs a versioned map that also verifies its public root and quantization fields. Rows retain source type; published pages/catalogs are posted quotes, not API quotes or fills. Other provider pages remain raw evidence only. |
...
| `market-capture` | `market_quotes`, `market_counterfactual_quotes` | hourly workflow | Includes GeckoTerminal indexed state as a reserve/volume control, fixed-notional Uniswap QuoterV2 simulations, and exact-input parent-block AMM counterfactuals for observed CoW USDC sells. Neither simulation is a fill guarantee, executable depth, or best-execution result. |
