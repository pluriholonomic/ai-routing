...
| `capture-direct` | `direct_prices_daily` | daily | DeepInfra and Cerebras structured public model APIs, Groq and Together exact-ID docs tables, and two exact-ID Fireworks serverless model pages. Cerebras retains both its API ID and a first-party canonical Hugging Face key when supplied. Rows retain source type; published pages are posted quotes, not API quotes or fills. Other provider pages remain raw evidence only. |
