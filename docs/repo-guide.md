...
| `market-capture` | `market_quotes`, `market_counterfactual_quotes`, `uniswap_tick_book`, `akash_market_open_bids`, `nosana_job_activity` | hourly workflow | Includes GeckoTerminal indexed state as a reserve/volume control, six-point Uniswap QuoterV2 price-impact curves plus 100/500-bps sparse-ladder capacity lower bounds, a full finality-block-pinned initialized-tick state map for the registered Uniswap pools, exact-input parent-block AMM counterfactuals for observed CoW USDC sells, block-pinned open GPU bids from the current Akash live-GPU-provider universe, and aggregate-only Nosana job state/count/duration controls. Tick state and lower bounds are not full dollar depth; neither simulation nor an Akash bid is a fill guarantee or best-execution result; Nosana activity is not LLM routing, verified GPU-hours, or a capacity measure. |
