...
| `capture-direct` | `direct_prices_daily` | daily | DeepInfra's structured public model API, Together's public serverless docs table, and two exact-ID Fireworks serverless model pages. Rows retain source type; published pages are posted quotes, not API quotes or fills. Other provider pages remain raw evidence only. |
